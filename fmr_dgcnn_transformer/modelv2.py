...
def get_graph_feature(x, k=20, large_k=None): #dgcnn-get_graph_feature
    # x = x.squeeze()
    if large_k is None:
...
class DGCNN(nn.Module):
    ...
    def forward(self, x):
        batch_size, num_dims, num_points = x.size() #dgcnn start
...
class DCP(nn.Module):
    ...
    def forward(self, *input):
        src = input[0]
        tgt = input[1]
        src_embedding = self.emb_nn(src) #emb_nn = DGCNN
        tgt_embedding = self.emb_nn(tgt)
        iter=input[2]

        src_embedding_p, tgt_embedding_p = self.pointer(src_embedding, tgt_embedding) #transformer
